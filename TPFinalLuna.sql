...
create database if not exists negocio_ropa;
use negocio_ropa;
...
select count(id_cliente) from clientes; -- 1

select 		count(email) cantidad, apellido, nombre 
from 		clientes
where 		email is not null
group by 	apellido, nombre 
order by	apellido, nombre desc;-- 2

select * from clientes where apellido like '%z'; -- 3

select count(legajo) from vendedores; -- 4

select * from vendedores where nombre like '_a%'; -- 5

select * from clientes where id_cliente not in (select id_cliente from facturas); -- 6

select 		fecha, count(numero) cantidad_de_facturas
from 		facturas 
group by 	fecha; -- 7 

select id_cliente, nombre, apellido from clientes join articulos where producto like 'Remera%'; -- 8

select 		producto 
from 		articulos 
where 		codigo 
not in (select codigo from ventas); -- 9 

select 		concat(year(fecha),'/', month(fecha)) mes, 
			sum(monto) total
from 		facturas
group by 	mes
order by 	total desc; -- 10

select 		v.codigo, a.producto, sum(v.cantidad) cantidad
from 		ventas v
left join 	articulos a
on 			v.codigo = a.codigo
group by 	v.codigo; -- 11

select 		c.id_cliente, c.nombre, c.apellido
from 		clientes c
left join 	facturas f
on 			c.id_cliente = f.id_cliente
where 		f.fecha=(select min(fecha) from facturas);-- 12

select 		f.id_cliente, c.nombre, c.apellido, 
			count(f.id_cliente) cantidad_de_facturas
from 		facturas f
left join 		clientes c
on 			f.id_cliente = c.id_cliente
group by 	f.id_cliente; -- 13

alter table articulos add column precio_costo double after color; -- 14.1
alter table articulos add column precio_venta double after precio_costo; -- 14.2
describe articulos;

set sql_safe_updates= 0;
select * from articulos;
update articulos set stock_minimo = 100 where stock_minimo >= 0; -- 15
update articulos set stock_maximo = 500 where stock_maximo <= 500; -- 15
select * from articulos;

delete from articulos where codigo not in (select codigo from ventas); -- 16

select nombre, apellido from clientes where email like '%gmail%'
union all
select nombre, apellido from vendedores where email like '%gmail%'; -- 17

select 		producto, stock mayor_stock 
from 		articulos 
where 		stock = (select max(stock) from articulos); -- 18
